...
const Navbar = ({ cartCount = 0 }) => {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      ...
      try {
        const response = await categoryAPI.getCategoriesPublic();
        if (response.success && response.data) {
          // Show all categories (parents and children) as flat list
          const categories = response.data
            ...
            .map(cat => ({
              ...
              image: cat.imageUrl || cat.iconUrl
            }));
...
//             image: parent.imageUrl || parent.iconUrl,
//             hasChildren: categories.some(cat => cat.parentCategoryId === parent.id),
//             children: categories
...
//                 image: child.imageUrl || child.iconUrl,
//                 hasChildren: categories.some(cat => cat.parentCategoryId === child.id),
//                 children: categories
//                   .filter(cat => cat.parentCategoryId === child.id)
//                   .sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0))
//                   .map(grandchild => ({
//                     id: grandchild.id,
...
//                     image: grandchild.imageUrl || grandchild.iconUrl
//                   }))
//               }))
//           }));

//           setMenuItems(categoryTree);
//         }
//       } catch (error) {
//         console.error('Failed to fetch categories:', error);
//         // Fallback menu items if API fails
